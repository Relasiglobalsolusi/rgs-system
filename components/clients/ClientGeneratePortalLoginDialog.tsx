...
function formatBulkResultMessage(
  result: Awaited<ReturnType<typeof generateClientPortalLogins>>,
...
export default function ClientGeneratePortalLoginDialog({
  open,
  onOpenChange,
  selectedCount,
  selectedIds,
  ...
  function handleConfirm() {
    startTransition(async () => {
      try {
        const result = await generateClientPortalLogins(selectedIds);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <EmployeeDialogShell
        icon={KeyRound}
        title={t("pages.users.generateClientTitle")}
