...
import { bulkReactivateEmployees } from "@/app/employees/actions";
import {
  EmployeeDialogShell,
...
export default function EmployeeBulkReactivateDialog({
  ...
  function handleConfirm() {
    startTransition(async () => {
      try {
        const result = await bulkReactivateEmployees(selectedIds);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <EmployeeDialogShell
        icon={RotateCcw}
        title={t("pages.employees.bulkRestoreTitle", { count: selectedCount })}
          ...
          <div className="flex w-full flex-col gap-3 sm:flex-col">
            ...
            >
              {pending
                ...
                : t("pages.employees.bulkRestoreConfirm", {
                    count: selectedCount,
                  })}
            </EmployeePrimaryButton>
