...
import type { ReportClientRow } from "@/app/reports/actions";
...
export default function ReportClientDirectory({ clients }: Props) {
  const { t } = useT();
  ...
  if (clients.length === 0) {
    return (
      <EmptyState
        title={t("pages.reports.noClients")}
        description={t("pages.reports.noClientsDesc")}
      />
    );
  }
  ...
  return (
    <div className="space-y-4">
      <DirectorySearchInput
        ...
        placeholder={t("pages.reports.searchClients")}
      />

      {visible.length === 0 ? (
        <EmptyState
          title={t("common.labels.noResults")}
          description={t("pages.reports.noClientsMatch")}
        />
      ) : (
