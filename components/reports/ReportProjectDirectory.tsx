...
import type { ReportProjectRow } from "@/app/reports/actions";
...
export default function ReportProjectDirectory({
  clientId,
  projects,
}: Props) {
  const { t, locale } = useT();
  ...
  if (projects.length === 0) {
    return (
      <EmptyState
        title={t("pages.reports.noProjects")}
        description={t("pages.reports.noProjectsDesc")}
      />
    );
  }
    ...
    <div className="space-y-4">
      ...
      {visible.length === 0 ? (
        <EmptyState
          ...
          description={t("pages.reports.noProjectsMatch")}
        />
      ) : (
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {visible.map((project) => {
            ...
            return (
              <Link
                key={project.id}
                href={`/reports/${clientId}/${project.id}`}
