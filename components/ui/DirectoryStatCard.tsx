...
export default function DirectoryStatCard({
  ...
  const card = (
    <div
      className={cn(
        "flex h-full min-w-0 flex-col overflow-hidden rounded-2xl border border-border bg-card transition duration-300",
        compact ? "px-3 py-2" : featured ? "px-6 py-5" : "px-5 py-4",
        interactive &&
        ...
        {align === "center" && icon ? (
          <div
            className={cn(
              ...
              compact ? "h-7 w-7" : featured ? "h-11 w-11" : "h-10 w-10",
              cardTintIcon[accent]
            )}
          >
            {icon}
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <p
            className={cn(
              ...
              "text-text",
              compact
                ? "mt-0.5 text-base leading-tight sm:text-lg xl:text-xl"
                : featured
          ...
          {subtitle ? (
            <p
              className={cn(
                "mt-auto text-pretty text-subtle",
                compact
                  ? "line-clamp-2 pt-1.5 text-[11px] leading-snug"
                  : "pt-3 text-xs leading-snug"
              )}
            >
              {subtitle}
            </p>
