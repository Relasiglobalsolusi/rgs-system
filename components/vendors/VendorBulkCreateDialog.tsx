...
import { createVendorsInBulk } from "@/app/vendors/actions";
...
export default function VendorBulkCreateDialog({
  open: controlledOpen,
  onOpenChange,
}: Props) {
  const { t } = useT();
  ...
  function submit(formData: FormData) {
    ...
    startTransition(async () => {
      try {
        await createVendorsInBulk(formData);
  ...
  return (
    <>
      ...
      >
        <EmployeeDialogShell
          icon={Truck}
          title={t("pages.vendors.bulkCreateTitle")}
          description={t("pages.vendors.bulkCreateDesc")}
          maxWidth="lg"
          footer={
        ...
        >
          ...
          >
            ...
            <BulkLineList
              title={t("pages.vendors.bulkCreateLines")}
              description={t("pages.vendors.bulkCreateLinesHint")}
              lineKeys={lineKeys}
              onAdd={(count) =>
                setLineKeys((current) => [
                  ...current,
