  ...
  inventory: bilingual(
    {
      ...
      steps: [
        "Open Inventory. Use the tabs to pick a view. Predicted km per litre and tank size are set on the vehicle type in Goods Catalog and do not change on the plate. The first fuel fill records litres added as the starting leftover. Later fills then show leftover fuel from kilometers driven.",
    ...
    },
    {
      ...
      steps: [
        "Buka Inventaris. Pakai tab untuk memilih tampilan. Prediksi km per litre dan ukuran tangki diisi pada tipe kendaraan di Katalog Barang dan tidak diubah pada plat. Pengisian BBM pertama mencatat litre yang ditambah sebagai sisa awal. Pengisian berikutnya lalu menampilkan sisa BBM dari kilometer yang ditempuh.",
  ...
  if (audience === "client") {
    if (locale === "id") {
      return {
        ...
        steps: [
          ...
          "Klik tombol [Pertama Kali Masuk].",
          ...
          "Isi [Email Pemulihan]. Dipakai hanya jika Anda menekan tombol [Lupa Kata Sandi].",
          "Klik [Simpan Dan Masuk]. Setelah itu Anda melihat catatan organisasi Anda saja.",
          "Kunjungan berikutnya: masuk dengan [Username] dan kata sandi itu. Jangan memakai tombol [Pertama Kali Masuk] lagi.",
          "Jika muncul [Akun Sudah Masuk], pilih [Batal] atau [Tutup Sesi Lain]. Hanya satu perangkat yang boleh tetap masuk.",
          "Jika lupa kata sandi, klik tombol [Lupa Kata Sandi] dan pakai [Username]. Tautan reset dikirim ke [Email Pemulihan].",
        ],
        remember: [
    ...
    return {
      ...
      steps: [
        ...
        "Click the [First Time Signing In] button.",
        ...
        "Enter [Recovery Email]. It is used only when you click the [Forgot Password] button.",
        "Click [Save And Sign In]. After that you see only your organization's records.",
        "Later visits: sign in with [Username] and that password. Do not use the [First Time Signing In] button again.",
        "If you see [Account Already Signed In], choose [Cancel] or [Close Other Session]. Only one device can stay signed in.",
        "If you forget the password, click the [Forgot Password] button and use your [Username]. The reset link goes to [Recovery Email].",
      ],
      remember: [
